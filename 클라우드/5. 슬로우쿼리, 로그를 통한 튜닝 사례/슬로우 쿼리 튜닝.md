# 1. 로그 수집
작업서버(VMS, 오케스트라 서버)에서 cron으로 매일 NCP Cloud DB(MySQL)의 Slow/Error 로그를 Object Storage로 Export하고, VMS로 다운로드하여 전일 Slow 로그를 병합·요약(mysqldumpslow)하는 일일 관측 파이프라인을 운영했다.
- 운영/장애 분석의 1차 근거가 되는 Slow/Error 로그를 **환경별(웹전시/PRD/STG)·일자별로 표준 수집**
- DB 서버 OS에 직접 접속하지 않고, **NCP 공식 기능(Export to Object Storage)** 기반으로 수집하여 접근통제/감사 관점에 유리한 운영 체계 확보
- 전일 Slow 로그를 자동 요약하여 **“튜닝 후보군”을 매일 생산**(운영자의 루틴 자동화)
...
Cloud DB Instances
  ...
  └─(NCP CLI: exportDbServerLogToObjectStorage / logType=SLOW,ERROR)
       ...
       └─ Object Storage (prefix = {ENV}/{YYYYMMDD}/)
       
            └─(VMS: list-objects → download)
            
                 └─ Local FS (/backup/DB_logs/…/{YYYYMMDD}/…)
                 
                      └─(mysqldumpslow)
                      
                           └─ Daily Report (Top SQL candidates)
...
| 시간 | 작업 | 스크립트 | 결과 |
|---:|---|---|---|
| 23:30 | WEB 전시 DB 로그 수집(Export+Download) | `db_log.sh` | 전일 분석용 raw 로그 적재 |
| 23:35 | PRD 운영 DB 로그 수집(Export+Download) | `db_prd_log.sh` | PRD raw 로그 적재 |
| 23:53 | STG 2026 로그 수집(Export+Download) | `2026_stg_log.sh` | STG raw 로그 적재 |
| 08:30 | 전일 PRD slow 병합/요약 리포트 생성 | `slow_sort.sh` | 튜닝 후보 리포트 산출 |

###  `slow_sort.sh`에서 집계된 튜닝 후보들 중 COUNT 횟수, ROW_Ratio 비율 등을 산정하여 튜닝 쿼리를 지정한다.


# 4. Log로 수집된 Slow 쿼리 튜닝 전 원문 쿼리
[원문]
```
SELECT
...
## 5-2. 개선 후 구조(TO-BE v2)에서 EXPLAIN이 보여주는 개선
...
## D. 남아있는 Using temporary, Using filesort의 의미
