...
## 6) 산출물(증빙) – GitBook 첨부 가이드

아래 파일/이미지를 GitBook 페이지에 함께 첨부해 **“작동 증빙”**을 완성했다.

### A. 작업 스케줄러 등록 증빙

* `백업삭제스케줄(윈도우).png`

  * 작업 스케줄러 내 등록된 작업명, 실행 주기, 마지막 실행 결과, 실행 커맨드가 보이도록 캡처

> 첨부:
> ![Windows Task Scheduler 설정](./백업삭제스케줄\(윈도우\).png)
...
현재 자동 스크립트는 “이동 후 즉시 삭제” 구조이므로,

* 운영 감사/사고 대응 관점에서 **격리 보관(Quarantine) 기간을 둔 후 삭제**가 더 안전하다.
* 개선안:
