...
## 10) 감사/면접관 관점 “증빙 포인트” 체크리스트

* [ ] **근거 정책(지침) → 목표(RPO/RTO) → 실행 절차(Runbook) → 리허설 결과(작업결과서/시뮬레이션)** 흐름이 연결되는가? 
* [ ] Failover 테스트에서 **전환 + DML 검증 + 원복**까지 수행했는가?
* [ ] 복구 시나리오에서 **NORECOVERY / LOG 체인 / STOPAT** 등 실무 디테일이 포함되는가?
* [ ] “시간”은 실제 측정값 또는 지침 준수 기준으로 **표 형태로 기입**되는가? 

---

## 11) (작성 예정) 리허설 타임라인 표

> 네가 시간은 채운다고 했으니 템플릿만 둠

| 구분                       |   시작 |   종료 |   소요 | 비고                          |
| ------------------------ | ---: | ---: | ---: | --------------------------- |
| Failover(Active→Standby) | [작성] | [작성] | [작성] | 리소스 인계 포함                   |
| DML 검증                   | [작성] | [작성] | [작성] | SELECT/INSERT/UPDATE/DELETE |
| Failback(Standby→Active) | [작성] | [작성] | [작성] | 원복                          |
| DR 복구(Full+Log+StopAt)   | [작성] | [작성] | [작성] | NORECOVERY/RECOVERY         |
| 최종 검증/모니터링               | [작성] | [작성] | [작성] | 알람/로그 확인                    |

---

# 더 필요한 자료(있으면 좋고, 없어도 작성은 가능)

너가 “더 필요한 게 있으면 말해”라고 했으니, **증빙 퀄리티를 한 단계 올리려면** 아래 3개가 있으면 좋다(없어도 위 문서는 충분히 제출 가능).

1. Failover 당시 **클러스터 관리자 화면/리소스 그룹 이동** 캡처(없다면 생략 가능)
2. 복구 후 검증 쿼리 결과(예: `state_desc=ONLINE`, 핵심 테이블 조회 결과) 캡처 1장
3. 백업 스케줄(크론/에이전트/작업 스케줄러) 설정 화면 또는 스크립트 원문

원하면, 다음 턴에 내가 위 문서에서 **“포트폴리오 한 페이지 요약(임팩트 중심)”** 버전도 같이 만들어줄게.
