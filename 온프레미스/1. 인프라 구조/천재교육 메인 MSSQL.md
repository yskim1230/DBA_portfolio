...
## 1) 단일 DB가 아니라 다수 DB로 분리된 대규모 교육 플랫폼 구조

본 MSSQL 서버는 약 79개 DB로 분리 운영되고 있으며, DB별로 기능이 강하게 분화되어 있습니다(회원/SSO/메시징/로그/서비스별 도메인).
...
서비스 트랜잭션은 각 도메인 DB에서 발생하되 **공통 기능(SSO, 회원, 메시징, 로그)이 중앙 축처럼 재사용**되는 구조입니다.

---

## 3) DB 군별 역할 분해 (운영 관점의 도메인 맵)
...
* **TSherpa**
  ...
  * 교수 사이트,회원(서비스 레벨), 커뮤니티(게시판/댓글/추천), 알림, 콘텐츠 소비/활동 기록 등 “서비스 기능”이 응집된 DB로 해석
* **Chunjae_Main**
...
* **Chunjae_SMS** (+ “SMSAgentCJ*”류 DB들)
  ...
  * 회원에게 안내 문자 발송용. /템플릿/발송 이력/게이트웨이 상태 등 메시지 파이프라인은 에이전트를 이용한 분리 운영
...
* 교육 서비스에서 흔한 유료 콘텐츠/교재/서비스 결제 도메인이 Main에 포함, 최소한 주문 허브가 Main에 존재

### (4) 메시징(SMS) 계열

* 발송 마스터/발송 이력/템플릿류 테이블 구조가 분리
* 운영 포인트: 재발송/중복발송 방지키, 트랜잭션 일관성, 대량 발송 시 큐 관리

---

## 5) 리스크 체크리스트(SQL Server)
...
2. **로그/이력(Log) 영역**
   ...
   * 증적관점에서 무엇을 남기는지(접근/변경/다운로드/발송 등)
...
4. **서비스 코어(TSherpa/Main)**
   ...
   * DB 분리가 되어 있어도, 공통 엔터티(회원/권한) Join이 잦으면 병목이 발생 가능성
