...
```sql
WITH cte AS (SELECT id, parent_id, name, 1 AS lv, CONVERT(nvarchar(1000), name) AS cate
             FROM cms_category
             WHERE code = 'E-curri_PA-music'
               AND site_id = 2
               AND disabled = 0
             ...
             SELECT c2.id,
                    ...
                    c2.name,
                    c1.lv + 1                                                                            AS lv,
                    CONVERT(nvarchar(1000), CONCAT(c1.cate, '>', c2.name))                               AS cate
             FROM cte c1
                , cms_category c2
             WHERE c2.site_id = 2
               AND c2.disabled = 0
               AND c1.id = c2.parent_id
               AND c2.id != ISNULL(c1.parent_id, '')
		   )

   , contentview AS (SELECT content_path, user_id, content_id, '25-07' AS ym
                     FROM tsherpalog2021.dbo.tbl_contentview_log_202507
