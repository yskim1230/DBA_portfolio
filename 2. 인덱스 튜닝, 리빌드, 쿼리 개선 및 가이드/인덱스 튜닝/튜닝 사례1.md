...
최종 실행되는 USP_Schedule_MemberPoint_RP 에서 부하가 발생된다고 판단했으며, 해당 SP 실행시 과도한 연산 작업으로 오류가 발생한다고 추측.
